...
import 'dart:async';
class GPAcalc6 extends StatefulWidget {
  final int n6;

  GPAcalc6(this.n6);

  @override
  GPAcalcstate6 createState() => new GPAcalcstate6();
}

class GPAcalcstate6 extends State<GPAcalc6> {
  List<String> _items = ['O', 'A+', 'A', 'B+', 'B', 'C', 'RA'].toList();
  ...
  void initState() {
    super.initState();
    _selection= new List<String>()..length=widget.n6;
    _selectionCp= new List<String>()..length=widget.n6;
    list = new List<int>.generate(widget.n6, (i) =>i);
  }
          ...
          if(safeToNavigate)
            Navigator.of(context).pushReplacement(
              new MaterialPageRoute(
                builder: (BuildContext context) => new GPA6(res6),
              ),
            );
          else{
            alert();
